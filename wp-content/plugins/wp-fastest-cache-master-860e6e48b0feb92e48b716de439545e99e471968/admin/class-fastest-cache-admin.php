...
class Fastest_Cache_Admin {
	 ...
	 */
	const WP_CLI_COMMAND = 'fastest-cache';
	...
	public function __construct( $plugin_name, $version ) {
		...
		$this->settings_tabs = apply_filters(
			'rt_fastest_cache_settings_tabs',
			array()
		);

		$this->options = $this->fastest_cache_settings();
	...
	public function enqueue_styles( $hook ) {
		 ...
		 * defined in Fastest_Cache_Loader as all of the hooks are defined
		 * in that particular class.
		 *
		 * The Fastest_Cache_Loader will then create the relationship
		...
		wp_enqueue_style( $this->plugin_name, plugin_dir_url( __FILE__ ) . 'css/fastest-cache-admin.css', array(), $this->version, 'all' );
	...
	public function enqueue_scripts( $hook ) {
		...
		wp_enqueue_script( $this->plugin_name, plugin_dir_url( __FILE__ ) . 'js/fastest-cache-admin.js', array( 'jquery' ), $this->version, false );

		$do_localize = array(
			'purge_confirm_string' => esc_html__( 'Purging entire cache is not recommended. Would you like to continue?', 'fastest-cache' ),
		);
		wp_localize_script( $this->plugin_name, 'fastest_cache', $do_localize );
	 ...
	 * @since    2.0.0
	 */
	public function fastest_cache_admin_menu() {

		if ( is_multisite() ) {

			add_submenu_page(
				'settings.php',
				__( 'Fastest Cache', 'fastest-cache' ),
				__( 'Fastest Cache', 'fastest-cache' ),
				'manage_options',
				'nginx',
				array( &$this, 'fastest_cache_setting_page' )
			);
	 ...
	 */
	public function fastest_cache_toolbar_purge_link( $wp_admin_bar ) {
		...
		if ( is_admin() ) {
			...
			$link_title         = __( 'Purge Cache', 'fastest-cache' );
		} else {
			$fastest_cache_urls = 'current-url';
			$link_title         = __( 'Purge Current Page', 'fastest-cache' );
		}

		$purge_url = add_query_arg(
			array(
				'fastest_cache_action' => 'purge',
				'fastest_cache_urls'   => $fastest_cache_urls,
			)
		);
	 ...
	 * @since    2.0.0
	 */
	public function fastest_cache_setting_page() {
		include plugin_dir_path( __FILE__ ) . 'partials/fastest-cache-admin-display.php';
	}
	 ...
	 * @return array
	 */
	public function fastest_cache_default_settings() {
	 ...
	 * @since    2.0.0
	 */
	public function fastest_cache_settings() {

		$options = get_site_option(
			'rt_wp_fastest_cache_options',
			array(
				'redis_hostname' => '127.0.0.1',
				'redis_port'     => '6379',
				'redis_prefix'   => 'nginx-cache:',
		...
		$data = wp_parse_args(
			$options,
			$this->fastest_cache_default_settings()
		);

		$is_redis_enabled = (
			defined( 'RT_WP_FASTEST_CACHE_REDIS_HOSTNAME' ) &&
			defined( 'RT_WP_FASTEST_CACHE_REDIS_PORT' ) &&
			defined( 'RT_WP_FASTEST_CACHE_REDIS_PREFIX' )
		);
		...
		$data['redis_hostname']            = RT_WP_FASTEST_CACHE_REDIS_HOSTNAME;
		$data['redis_port']                = RT_WP_FASTEST_CACHE_REDIS_PORT;
		$data['redis_prefix']              = RT_WP_FASTEST_CACHE_REDIS_PREFIX;
	...
	/**
	 * Fastest cache setting link function.
	 *
	 * @param array $links links.
	 *
	 * @return mixed
	 */
	public function fastest_cache_settings_link( $links ) {
		...
		$settings_link = '<a href="' . network_admin_url( $setting_page . '?page=nginx' ) . '">' . __( 'Settings', 'fastest-cache' ) . '</a>';
	...
	public function functional_asset_path() {
		...
		$log_path = WP_CONTENT_DIR . '/uploads/fastest-cache/';
	...
	public function functional_asset_url() {
		...
		$log_url = WP_CONTENT_URL . '/uploads/fastest-cache/';
	 ...
	 * @since     1.0
	 */
	public function fastest_cache_get_feeds() {
		...
		<ul role="list">
			<?php
			if ( 0 === $maxitems ) {
				echo '<li role="listitem">' . esc_html_e( 'No items', 'fastest-cache' ) . '.</li>';
			} else {
						...
						<li role="listitem">
							<?php
								printf(
									...
									esc_attr__( 'Posted ', 'fastest-cache' ) . esc_attr( $item->get_date( 'j F Y | g:i a' ) ),
	...
	public function add_timestamps() {
		...
		$timestamps = "\n<!--" .
				'Cached using Fastest-Cache on ' . current_time( 'mysql' ) . '. ' .
	...
	public function set_future_post_option_on_future_status( $new_status, $old_status, $post ) {
		...
		if (
			...
			update_site_option( 'rt_wp_fastest_cache_options', $this->options );
	...
	public function update_new_blog_options( $blog_id ) {
		...
		$helper_options = $this->fastest_cache_default_settings();
		update_blog_option( $blog_id, 'rt_wp_fastest_cache_options', $helper_options );
	...
	public function purge_all() {
		...
		if ( 'POST' === $method ) {
			$action = filter_input( INPUT_POST, 'fastest_cache_action', FILTER_SANITIZE_STRING );
		} else {
			$action = filter_input( INPUT_GET, 'fastest_cache_action', FILTER_SANITIZE_STRING );
		}
		...
		check_admin_referer( 'fastest_cache-purge_all' );
		...
		if ( ! is_admin() ) {
			...
			$redirect_url = add_query_arg( array( 'fastest_cache_action' => 'done' ) );
		}
		...
		if ( 'purge' === $action ) {
		     ...
		     * @since 1.0
		     */
		    do_action( 'rt_fastest_cache_after_purge_all' );
	...
	public function display_notices() {
		echo '<div class="updated"><p>' . esc_html__( 'Purge initiated', 'fastest-cache' ) . '</p></div>';
	}
