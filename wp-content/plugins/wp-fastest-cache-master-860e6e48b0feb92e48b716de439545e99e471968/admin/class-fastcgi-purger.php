...
class FastCGI_Purger extends Purger {
	...
	public function purge_url( $url, $feed = true ) {
		 ...
		 */
		$url = apply_filters( 'rt_fastest_cache_purge_url', $url );
		...
		switch ( $fastest_cache_admin->options['purge_method'] ) {
	...
	public function custom_purge_urls() {
		...
		$purge_urls = isset( $fastest_cache_admin->options['purge_url'] ) && ! empty( $fastest_cache_admin->options['purge_url'] ) ?
			explode( "\r\n", $fastest_cache_admin->options['purge_url'] ) : array();
		 ...
		 */
		$purge_urls = apply_filters( 'rt_fastest_cache_purge_urls', $purge_urls, false );
	...
	public function purge_all() {
		...
		switch ( $fastest_cache_admin->options['purge_method'] ) {
			...
			case 'unlink_files':
				$this->unlink_recursive( RT_WP_FASTEST_CACHE_CACHE_PATH, false );
				$this->log( '* * * * *' );
		 ...
		 * @since 1.0
		 */
		do_action( 'rt_fastest_cache_after_fastcgi_purge_all' );
	}
	...
	private function purge_base_url() {
		 ...
		 * @since 1.0
		 */
		$path = apply_filters( 'rt_fastest_cache_fastcgi_purge_suffix', '/--api/cache-purge' );
		 ...
		 * @since 1.0
		 */
		$purge_url_base = apply_filters( 'rt_fastest_cache_fastcgi_purge_url_base', $purge_url_base );
