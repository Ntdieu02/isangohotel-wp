 ...
 * @package    fastest-cache
 *
 * @subpackage fastest-cache/admin
 *
 * @author     LWS
 */
abstract class Purger {
	...
	public function purge_post_on_comment_change( $newstatus, $oldstatus, $comment ) {
		...
		global $fastest_cache_admin, $blog_id;

		if ( ! $fastest_cache_admin->options['enable_purge'] ) {
			return;
		}
		...
		switch ( $newstatus ) {
			...
			case 'approved':
				if ( 1 === (int) $fastest_cache_admin->options['purge_page_on_new_comment'] ) {
			...
			case 'unapproved':
			case 'trash':
				if ( 'approved' === $oldstatus && 1 === (int) $fastest_cache_admin->options['purge_page_on_deleted_comment'] ) {
	...
	public function purge_post( $post_id ) {
		...
		if ( 1 === (int) $fastest_cache_admin->options['purge_homepage_on_edit'] ) {
		...
		if ( 'comment_post' === current_filter() || 'wp_set_comment_status' === current_filter() ) {
			...
			$this->_purge_by_options(
				$post_id,
				$blog_id,
				$fastest_cache_admin->options['purge_page_on_new_comment'],
				$fastest_cache_admin->options['purge_archive_on_new_comment'],
				$fastest_cache_admin->options['purge_archive_on_new_comment']
			);
			...
			$this->_purge_by_options(
				$post_id,
				$blog_id,
				$fastest_cache_admin->options['purge_page_on_mod'],
				$fastest_cache_admin->options['purge_archive_on_edit'],
				$fastest_cache_admin->options['purge_archive_on_edit']
			);
	...
	protected function delete_cache_file_for( $url ) {
		...
		if ( ! defined( 'RT_WP_FASTEST_CACHE_CACHE_PATH' ) ) {

			$this->log( 'Error purging because RT_WP_FASTEST_CACHE_CACHE_PATH was not defined. URL: ' . $url, 'ERROR' );
			return false;
		...
		$cache_path = RT_WP_FASTEST_CACHE_CACHE_PATH;
		 ...
		 */
		$cached_file = apply_filters( 'rt_fastest_cache_purge_cached_file', $cached_file );
		...
		if ( unlink( $cached_file ) ) {
			 ...
			 * @param string $cached_file Cached file name.
			 */
			do_action( 'rt_fastest_cache_purged_file', $url, $cached_file );
		} else {
	...
	protected function do_remote_get( $url ) {
		 ...
		 * @param string $url URL to be purged.
		 */
		$url = apply_filters( 'rt_fastest_cache_remote_purge_url', $url );
		 ...
		 * @param string $url URL to be purged.
		 */
		do_action( 'rt_fastest_cache_before_remote_purge_url', $url );
		...
		if ( is_wp_error( $response ) ) {
			 ...
			 */
			do_action( 'rt_fastest_cache_after_remote_purge_url', $url, $response );
		}
	...
	public function log( $msg, $level = 'INFO' ) {
		...
		if ( ! $fastest_cache_admin->options['enable_log'] ) {
			return;
		}
		...
		if ( $log_levels[ $level ] >= $log_levels[ $fastest_cache_admin->options['log_level'] ] ) {

			$fp = fopen( $fastest_cache_admin->functional_asset_path() . 'nginx.log', 'a+' );
			if ( $fp ) {
	...
	public function check_and_truncate_log_file() {
		...
		$nginx_asset_path = $fastest_cache_admin->functional_asset_path() . 'nginx.log';

		if ( ! file_exists( $nginx_asset_path ) ) {
			return;
		}

		$max_size_allowed = ( is_numeric( $fastest_cache_admin->options['log_filesize'] ) ) ? $fastest_cache_admin->options['log_filesize'] * 1048576 : 5242880;
	...
	public function purge_on_post_moved_to_trash( $new_status, $old_status, $post ) {
		...
		if ( 'trash' === $new_status ) {
			...
			if ( 1 === (int) $fastest_cache_admin->options['purge_homepage_on_del'] ) {
				$this->_purge_homepage();
			}
			...
			$this->_purge_by_options(
				...
				$blog_id,
				true,
				$fastest_cache_admin->options['purge_archive_on_del'],
				$fastest_cache_admin->options['purge_archive_on_del']
			);
	...
	private function _purge_homepage() {
		...
		if ( function_exists( 'icl_get_home_url' ) ) {
			...
			$this->log( sprintf( __( 'Purging homepage (WPML) ', 'fastest-cache' ) . '%s', $homepage_url ) );

		} else {

			$homepage_url = trailingslashit( home_url() );
			$this->log( sprintf( __( 'Purging homepage ', 'fastest-cache' ) . '%s', $homepage_url ) );
	...
	private function _purge_personal_urls() {
		...
		$this->log( __( 'Purging personal urls', 'fastest-cache' ) );

		if ( isset( $fastest_cache_admin->options['purgeable_url']['urls'] ) ) {

			foreach ( $fastest_cache_admin->options['purgeable_url']['urls'] as $url ) {
				$this->purge_url( $url, false );
			}
		} else {
			$this->log( '- ' . __( 'No personal urls available', 'fastest-cache' ) );
		}
	...
	private function _purge_post_categories( $_post_id ) {
		...
		$this->log( __( 'Purging category archives', 'fastest-cache' ) );
		...
		if ( ! is_wp_error( $categories ) && ! empty( $categories ) ) {
			...
			foreach ( $categories as $category_id ) {
				...
				$this->log( sprintf( __( "Purging category '%d'", 'fastest-cache' ), $category_id ) );
				$this->purge_url( get_category_link( $category_id ) );
	...
	private function _purge_post_tags( $_post_id ) {
		...
		$this->log( __( 'Purging tags archives', 'fastest-cache' ) );
		...
		if ( ! is_wp_error( $tags ) && ! empty( $tags ) ) {
			...
			foreach ( $tags as $tag ) {
				...
				$this->log( sprintf( __( "Purging tag '%1\$s' ( id %2\$d )", 'fastest-cache' ), $tag->name, $tag->term_id ) );
				$this->purge_url( get_tag_link( $tag->term_id ) );
	...
	private function _purge_post_custom_taxa( $_post_id ) {
		...
		$this->log( __( 'Purging post custom taxonomies related', 'fastest-cache' ) );
		...
		if ( ! empty( $custom_taxonomies ) ) {
			...
			foreach ( $custom_taxonomies as $taxon ) {
				...
				$this->log( sprintf( '+ ' . __( "Purging custom taxonomy '%s'", 'fastest-cache' ), $taxon ) );
				...
				if ( ! in_array( $taxon, array( 'category', 'post_tag', 'link_category' ), true ) ) {
					...
					// translators: %s: Post taxonomy name.
					$this->log( sprintf( '- ' . __( "Your built-in taxonomy '%s' has param '_builtin' set to false.", 'fastest-cache' ), $taxon ), 'WARNING' );
				}
			}
		} else {
			$this->log( '- ' . __( 'No custom taxonomies', 'fastest-cache' ) );
		}
	...
	private function _purge_all_categories() {
		...
		$this->log( __( 'Purging all categories', 'fastest-cache' ) );
		...
		if ( ! empty( $_categories ) ) {
			...
			foreach ( $_categories as $c ) {
				...
				$this->log( sprintf( __( "Purging category '%1\$s' ( id %2\$d )", 'fastest-cache' ), $c->name, $c->term_id ) );
				$this->purge_url( get_category_link( $c->term_id ) );

			}
		} else {

			$this->log( __( 'No categories archives', 'fastest-cache' ) );
	...
	private function _purge_all_posttags() {
		...
		$this->log( __( 'Purging all tags', 'fastest-cache' ) );
		...
		if ( ! empty( $_posttags ) ) {
			...
			foreach ( $_posttags as $t ) {
				...
				$this->log( sprintf( __( "Purging tag '%1\$s' ( id %2\$d )", 'fastest-cache' ), $t->name, $t->term_id ) );
				$this->purge_url( get_tag_link( $t->term_id ) );

			}
		} else {
			$this->log( __( 'No tags archives', 'fastest-cache' ) );
		}
	...
	private function _purge_all_customtaxa() {
		...
		$this->log( __( 'Purging all custom taxonomies', 'fastest-cache' ) );
		...
		if ( ! empty( $custom_taxonomies ) ) {
			...
			foreach ( $custom_taxonomies as $taxon ) {
				...
				if ( ! in_array( $taxon, array( 'category', 'post_tag', 'link_category' ), true ) ) {
					...
					// translators: %s: Taxonomy name.
					$this->log( sprintf( '- ' . esc_html__( "Your built-in taxonomy '%s' has param '_builtin' set to false.", 'fastest-cache' ), $taxon ), 'WARNING' );
				}
			}
		} else {
			$this->log( '- ' . __( 'No custom taxonomies', 'fastest-cache' ) );
	...
	private function _purge_all_posts() {
		...
		$this->log( __( 'Purging all posts, pages and custom post types.', 'fastest-cache' ) );
		...
		if ( ! empty( $_posts ) ) {
			...
			foreach ( $_posts as $p ) {
				...
				$this->log( sprintf( '+ ' . __( "Purging post id '%1\$d' ( post type '%2\$s' )", 'fastest-cache' ), $p->ID, $p->post_type ) );
				$this->purge_url( get_permalink( $p->ID ) );

			}
		} else {
			$this->log( '- ' . __( 'No posts', 'fastest-cache' ) );
		}
	...
	private function _purge_all_date_archives() {
		...
		$this->log( __( 'Purging all date-based archives.', 'fastest-cache' ) );
	...
	private function _purge_all_daily_archives() {
		...
		$this->log( __( 'Purging all daily archives.', 'fastest-cache' ) );
		...
		if ( ! empty( $_daily_archives ) ) {
			...
			foreach ( $_daily_archives as $_da ) {
				...
				$this->log(
					sprintf(
						'+ ' . __( "Purging daily archive '%1\$s/%2\$s/%3\$s'", 'fastest-cache' ),
						$_da->year,
						$_da->month,
						$_da->dayofmonth
					)
			...
			$this->log( '- ' . __( 'No daily archives', 'fastest-cache' ) );
		}
	...
	private function _purge_all_monthly_archives() {
		...
		$this->log( __( 'Purging all monthly archives.', 'fastest-cache' ) );

		$_monthly_archives = wp_cache_get( 'fastest_cache_monthly_archives', 'fastest-cache' );
		...
		if ( empty( $_monthly_archives ) ) {
			...
			wp_cache_set( 'fastest_cache_monthly_archives', $_monthly_archives, 'fastest_cache', 24 * 60 * 60 );
		...
		if ( ! empty( $_monthly_archives ) ) {
			...
			foreach ( $_monthly_archives as $_ma ) {
				...
				$this->log( sprintf( '+ ' . __( "Purging monthly archive '%1\$s/%2\$s'", 'fastest-cache' ), $_ma->year, $_ma->month ) );
				$this->purge_url( get_month_link( $_ma->year, $_ma->month ) );

			}
		} else {
			$this->log( '- ' . __( 'No monthly archives', 'fastest-cache' ) );
		}
	...
	private function _purge_all_yearly_archives() {
		...
		$this->log( __( 'Purging all yearly archives.', 'fastest-cache' ) );

		$_yearly_archives = wp_cache_get( 'fastest_cache_yearly_archives', 'fastest_cache' );
		...
		if ( empty( $_yearly_archives ) ) {
			...
			wp_cache_set( 'fastest_cache_yearly_archives', $_yearly_archives, 'fastest_cache', 24 * 60 * 60 );
		...
		if ( ! empty( $_yearly_archives ) ) {
			...
			foreach ( $_yearly_archives as $_ya ) {
				...
				$this->log( sprintf( '+ ' . esc_html__( "Purging yearly archive '%s'", 'fastest-cache' ), $_ya->year ) );
				$this->purge_url( get_year_link( $_ya->year ) );

			}
		} else {
			$this->log( '- ' . __( 'No yearly archives', 'fastest-cache' ) );
		}
	...
	public function purge_on_term_taxonomy_edited( $term_id, $tt_id, $taxon ) {
		...
		$this->log( __( 'Term taxonomy edited or deleted', 'fastest-cache' ) );
		...
		if ( 'edit_term' === $current_filter && ! is_wp_error( $term ) && ! empty( $term ) ) {
			...
			$this->log( sprintf( __( "Term taxonomy '%1\$s' edited, (tt_id '%2\$d', term_id '%3\$d', taxonomy '%4\$s')", 'fastest-cache' ), $term->name, $tt_id, $term_id, $taxon ) );

		} elseif ( 'delete_term' === $current_filter ) {

			$this->log( sprintf( __( "A term taxonomy has been deleted from taxonomy '%1\$s', (tt_id '%2\$d', term_id '%3\$d')", 'fastest-cache' ), $taxon, $term_id, $tt_id ) );
	...
	public function purge_on_check_ajax_referer( $action ) {
		...
		switch ( $action ) {
			...
			case 'save-sidebar-widgets':
				$this->log( __( 'Widget saved, moved or removed in a sidebar', 'fastest-cache' ) );
				$this->_purge_homepage();
				break;
