...
class Predis_Purger extends Purger {
	...
	public function __construct() {
		...
		if ( ! class_exists( 'Predis\Autoloader' ) ) {
			require_once FASTEST_CACHE_BASEPATH . 'admin/predis.php';
		}
		...
		$this->redis_object = new Predis\Client(
			array(
				'host' => $fastest_cache_admin->options['redis_hostname'],
				'port' => $fastest_cache_admin->options['redis_port'],
			)
		);
	...
	public function purge_all() {
		...
		$prefix = trim( $fastest_cache_admin->options['redis_prefix'] );
		 ...
		 * @since 1.0
		 */
		do_action( 'rt_fastest_cache_after_redis_purge_all' );
	}
	...
	public function purge_url( $url, $feed = true ) {
		...
		$prefix          = $fastest_cache_admin->options['redis_prefix'];
		 ...
		 * This would need $url above to be changed with this filter `rt_fastest_cache_purge_url` by cache key that Nginx sets while generating cache.
		 *
		 ...
		 * ```
		 * add_filter( 'rt_fastest_cache_purge_url', function( $url ) {
		 *      $url = $url . '--*';
		 *      return $url;
		 * });
		 * ```
	...
	public function custom_purge_urls() {
		 ...
		 */
		$purge_urls = apply_filters( 'rt_fastest_cache_purge_urls', $purge_urls, true );
