...
if($cleverfox_theme->name =='Websy') {
	$cleverfox_file = CLEVERFOX_PLUGIN_URL .'inc/websy/images/logo.png';
	$cleverfox_ImagePath = CLEVERFOX_PLUGIN_URL .'inc/websy/images';
} elseif($cleverfox_theme->name =='Webora') {
	$cleverfox_file = CLEVERFOX_PLUGIN_URL .'inc/webora/images/logo.png';
	$cleverfox_ImagePath = CLEVERFOX_PLUGIN_URL .'inc/webora/images';
} elseif($cleverfox_theme->name =='WebAura') {
	$cleverfox_file = CLEVERFOX_PLUGIN_URL .'inc/webaura/images/logo.png';
	$cleverfox_ImagePath = CLEVERFOX_PLUGIN_URL .'inc/webaura/images';
} else {
	$cleverfox_file = CLEVERFOX_PLUGIN_URL .'inc/webique/images/logo.png';
	$cleverfox_ImagePath = CLEVERFOX_PLUGIN_URL .'inc/webique/images';
}
...
foreach($cleverfox_images as $cleverfox_name) {
    ...
    if (!is_wp_error($cleverfox_remote_content) && $cleverfox_remote_content['response']['code'] === 200) {
        ...
        if (!$cleverfox_upload_file['error']) {
            ...
            $cleverfox_attachment = array(
                ...
                'post_title' => preg_replace('/\.[^.]+$cleverfox_/', '', $cleverfox_filename),
 ...
 update_option( 'webique_media_id', $cleverfox_ImageId );
