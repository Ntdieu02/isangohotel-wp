...
if ( version_compare( $GLOBALS['wp_version'], '5.8-alpha-1', '<' ) ) {
    add_filter( 'block_categories', 'nexcraft_block_categories', 10, 2 );
} else {
    add_filter( 'block_categories_all', 'nexcraft_block_categories', 10, 2 );
}

// Register Blocks
function nexcraft_wp_register_script( $block, $options = array() ) {
    register_block_type( 'info-box/' . $block,
        array_merge( array(
            'editor_script' => 'nexcraft_editor_script',
            'editor_style'  => 'nexcraft_editor_style',
            'script'        => 'nexcraft_script',
            'style'         => 'nexcraft_style',
        ), $options )
    );
}

// Enqueue assets
function nexcraft_enqueue_block_assets() {
    wp_enqueue_script( 'font-awesome-kit', CLEVERFOX_PLUGIN_URL . '/inc/nexcraft/block/assets/js/font-awesome-kit.js', array(), '1.0', true );
}
add_action( 'enqueue_block_assets', 'nexcraft_enqueue_block_assets' );

function nexcraft_register() {
    // Resister script in editor
	wp_register_script( 'nexcraft_editor_script', CLEVERFOX_PLUGIN_URL . 'inc/nexcraft/block/dist/editor.js', array( 'wp-blob', 'wp-block-editor', 'wp-blocks', 'wp-components', 'wp-compose', 'wp-data', 'wp-element', 'wp-html-entities', 'wp-i18n', 'wp-rich-text', 'font-awesome-kit' ), '1.0', false ); 
...
// Generate Styles
class  NexCraftInfoStyleGenerator {
...
function nexcraft_render_nexcraft_infos($attributes, $content){
    ...
    // Generate Styles
    $infosStyle = new  NexCraftInfoStyleGenerator();
...
function nexcraft_render_nexcraft_info( $attributes ) {
    ...
    // Generate Styles
    $infoStyle = new  NexCraftInfoStyleGenerator();
    $infoStyle::addStyle("#nexcraftInfo-$cId .nexcraftInfo .icon", array(
    ...
    // echo "<div class='av-column-".$columns['desktop']." ".$infoTypes."' id='nexcraftInfo-$cId'>
	...
	echo "<div class='av-column-" . esc_attr($columns['desktop']) . " " . esc_attr($infoTypes) . "' id='nexcraftInfo-" . esc_attr($cId) . "'>
        <aside class='widget widget-contact'>
            <div class='contact-area'>
