...
( function( $ ) {
	...
	ARAVALLICustomizer = {
		...
		init: function()
		{
			ARAVALLICustomizer._initToggles();
		},
		...
		_initToggles: function()
		{
			...
			ARAVALLIControlTrigger.triggerHook( 'aravalli-toggle-control', api );

			// Loop through each setting.
			$.each( ARAVALLICustomizerToggles, function( settingId, toggles ) {
						...
						$.each(toggles, function( i, toggle) {
							...
							$.each(toggle.controls, function( k, controlId) {
								...
								if ( typeof parent != 'undefined' ) {
									...
									if ( ! visibleIt ) {
										$( '.control-section[aria-owns="' + parentId + '"]' ).addClass( 'aravalli-hide' );
									} else {
										$( '.control-section[aria-owns="' + parentId + '"]' ).removeClass( 'aravalli-hide' );
									}
								}
							});
						});
	...
	$( function() { ARAVALLICustomizer.init(); } );
